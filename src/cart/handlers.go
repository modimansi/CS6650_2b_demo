...
// Updated: Now uses CartStore interface for flexibility (PostgreSQL or DynamoDB)
type Handlers struct {
	store CartStore
}

// NewHandlers creates a new Handlers instance with the given store
func NewHandlers(store CartStore) *Handlers {
...
// Retrieves a cart with all its items using efficient JOINs (PostgreSQL) or single item read (DynamoDB)
func (h *Handlers) GetCart(c *gin.Context) {
	// Parse cart ID from path parameter
	cartIDStr := c.Param("shoppingCartId")

	// Try to parse as int first (PostgreSQL), otherwise use as string (DynamoDB UUID)
	var cartWithItems *CartWithItems
	var err error

	cartID, parseErr := strconv.Atoi(cartIDStr)
	if parseErr == nil && cartID >= 1 {
		// PostgreSQL style: integer ID
		cartWithItems, err = h.store.GetCartWithItems(cartID)
	} else {
		// DynamoDB style: UUID string
		cartWithItems, err = h.store.GetCartWithItems(cartIDStr)
	}
	...
	if err != nil {
		...
		log.Printf("Error retrieving cart %s: %v", cartIDStr, err)
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
func (h *Handlers) AddItemToCart(c *gin.Context) {
	...
	// Add or update item in cart (supports both int and string cart IDs)
	var err error
	cartID, parseErr := strconv.Atoi(cartIDStr)
	if parseErr == nil && cartID >= 1 {
		// PostgreSQL style: integer ID
		...
		// DynamoDB style: UUID string
		err = h.store.AddOrUpdateItem(cartIDStr, req.ProductID, req.Quantity)
	}
	...
	if err != nil {
		...
		log.Printf("Error adding item to cart %s: %v", cartIDStr, err)
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
func (h *Handlers) CheckoutCart(c *gin.Context) {
	...
	// Process checkout (supports both int and string cart IDs)
	...
	if parseErr == nil && cartID >= 1 {
		...
		// DynamoDB style: UUID string
		orderID, err = h.store.CheckoutCart(cartIDStr)
	}
	...
	if err != nil {
		...
		log.Printf("Error checking out cart %s: %v", cartIDStr, err)
		c.JSON(http.StatusInternalServerError, ErrorResponse{
